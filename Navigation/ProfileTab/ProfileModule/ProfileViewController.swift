...
class ProfileViewController: UIViewController {
    ...
    lazy var profileHeader = ProfileHeaderView(navigation: self)
...
extension ProfileViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case let n where n > 0:
            ...
            cell.addGestureRecognizer(viewModel.addToFavoriteGesture())
            cell.post = viewModel.outputPosts[indexPath.row-1]
            
            cell.isLikeButtonTap = { [weak self] post, isLiked in
                self?.viewModel.addToFavorite(model: post, isLiked: isLiked, index: indexPath.row-1)}
            return cell
        default:
